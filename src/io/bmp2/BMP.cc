...
uint BMPUtil::get_padded_stride(Bitmap& _bitmap)
{
...
uint BMPUtil::get_padded_stride_from_24bit_stride(uint _stride_24bit)
{
    uint padding_count;
...
uint BMPUtil::get_padded_stride_PX32RGBA(const Bitmap& _bitmap)
{
...
Vec<uint8_t> BMPUtil::pad_bitmap_data(Bitmap& _bitmap)
{
...
Vec<uint8_t> BMPUtil::remove_alpha(Bitmap& _bitmap)
{
...
Vec<uint8_t> BMPUtil::bitmap_to_bmp_buf(Bitmap& _bitmap)
{
    uint width_px = _bitmap.w();
...
uint BMPUtil::get_padding(uint _unpadded_stride_bytes)
{
    uint padding_count;
...
Vec<uint32_t> BMPUtil::RGB_to_RGBA_buf(Vec<uint8_t>& _RGB_buf)
{
...
Vec<uint8_t> BMPUtil::unpad_bitmap_data(Vec<uint8_t>& _padded_buf, uint _unpadded_stride, uint _bitmap_height)
{
...
// BMPFileInfoHeader::BMPFileInfoHeader()
// {

// }

BMPFileInfoHeader::BMPFileInfoHeader(const Bitmap& _bitmap)
{
    // set_BM();
...
void BMPFileInfoHeader::set_from_buffer(Vec<uint8_t> _buf)
{
    BM[0] = 'B';
    BM[1] = 'M';
...
void BMPFileInfoHeader::load_from_bitmap(const Bitmap& _bitmap)
{
    BM[0] = 'B';
    BM[1] = 'M';
...
Vec<uint8_t> BMPFileInfoHeader::get_header_buff()
{
...
void BMPFileInfoHeader::set_BM()
{
    buff[0] = 'B';
    buff[1] = 'M';
}
Vec<uint8_t> BMPFileInfoHeader::get_BM_buff()
{
...
void BMPFileInfoHeader::set_file_size(const Bitmap& _bitmap)
{
...
Vec<uint8_t> BMPFileInfoHeader::get_file_size_buff()
{
    return Vec<uint8_t>(4, (buff.data_mut()+2));
}
uint BMPFileInfoHeader::get_file_size()
{
    uint file_size = 0;
    file_size += (buff[2] << 24);
    file_size += (buff[3] << 16);
...
void BMPFileInfoHeader::set_reserved_1()
{
    buff[6] = 0x00;
    buff[7] = 0x00;
}
Vec<uint8_t> BMPFileInfoHeader::get_reserved_1_buff()
{
    return Vec<uint8_t>(2, (buff.data_mut()+6));
}

void BMPFileInfoHeader::set_reserved_2()
{
    buff[8] = 0x00;
    buff[9] = 0x00;
}
Vec<uint8_t> BMPFileInfoHeader::get_reserved_2_buff()
{
    return Vec<uint8_t>(2, (buff.data_mut()+8));
}

void BMPFileInfoHeader::set_first_pixel_location()
{
...
Vec<uint8_t> BMPFileInfoHeader::get_first_pixel_location_buff()
{
...
uint BMPFileInfoHeader::get_padded_BPM_stride(const Bitmap& _bitmap)
{
    uint stride_byte_count = _bitmap.w() * 3;
...
BMPDIBHeader::BMPDIBHeader(Bitmap& _bitmap)
{
    load_from_bitmap(_bitmap);
}

void BMPDIBHeader::load_from_bitmap(Bitmap& _bitmap)
{
    DIB_bytes = 40;
...
void BMPDIBHeader::set_from_buffer(Vec<uint8_t> _buf)
{
    memcpy(this, _buf.data(), 40);
}

Vec<uint8_t> BMPDIBHeader::get_DIB_buffer()
{
...
/** Returns number of bytes */
uint BMPDIBHeader::get_padded_BPM_stride(const Bitmap& _bitmap)
{
    uint stride_byte_count = _bitmap.w() * 3;
    uint padding_count = 4 - (stride_byte_count % 4);
    return stride_byte_count + padding_count;
...
void BMPIO::set_bitmap(Bitmap& _bitmap)
{
    bitmap = _bitmap;
...
void BMPIO::SExport_PX32RGBA(Str _file_path, Bitmap& _bitmap)
{
...
Bitmap BMPIO::SImport_PX32(Str _file_path)
{
...
Vec<uint8_t> BMPIO::unpad_bitmap_data(Vec<uint8_t>& _padded_buf)
{
...
Vec<uint8_t> BMPIO::pad_bitmap_data()
{
...
/** Returns number of bytes */
uint BMPIO::get_padding(uint _unpadded_stride_bytes)
{
    uint padding_count;
...
/** Returns number of bytes */
uint BMPIO::get_padding(uint bits_per_pixel, uint image_width)
{
...
/** Returns number of bytes */
uint BMPIO::get_padded_BPM_stride(const Bitmap& _bitmap)
{
    uint stride_byte_count = _bitmap.w() * 3;
